...
func buildOpentelemetry(c interface{}, s interface{}) string {
	...
	buf.WriteString(fmt.Sprintf("opentelemetry_propagate;\n"))
	// buf.WriteString(fmt.Sprintf("opentelemetry_capture_headers on;\n"))
	// buf.WriteString(fmt.Sprintf("opentelemetry_trust_incoming_spans on;\n"))

	// opentelemetry_propagate;
	// buf.WriteString(fmt.Sprintf("opentelemetry_operation_name \"%s\";\n", "test2"))
	// buf.WriteString(fmt.Sprintf("opentelemetry_config \"%s\";\n", cfg.OpentelemetryConfig))
	// buf.WriteString(fmt.Sprintf("opentelemetry_config \"%s\";\n", "/conf/otel-nginx.toml;"))
	return buf.String()
}
...
func buildOpentelemetryForLocation(isOTEnabled bool, isOTTrustSet bool, location *ingress.Location) string {
	...
	// opc = opc + "\nopentelemetry_operation_name new_op_name;"
	// opc = opc + "\nopentelemetry_trust_incoming_spans on;"
	opc = opc + "\nopentelemetry_capture_headers on;"
	opc = opc + "\nadd_header Context-TraceParent \"$opentelemetry_context_traceparent\";"
	// fmt.Println("########", location.Ingress.Name)
	fmt.Println("########", location.Opentelemetry)
	return opc
}
